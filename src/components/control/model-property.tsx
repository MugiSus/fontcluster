        ...
        itemComponent={(selectProps) => (
          <SelectItem item={selectProps.item} class='pr-8'>
            <span class='flex w-full min-w-0 items-center gap-2'>
              ...
              <Show when={selectProps.item.rawValue.availability === 'loading'}>
                <LoaderCircleIcon
              ...
              <Show when={selectProps.item.rawValue.availability === 'unknown'}>
                <CircleAlertIcon
                  class='size-3.5 shrink-0 text-muted-foreground'
          ...
          >
            {(state) => (
              <span class='flex min-w-0 items-center justify-end gap-1.5'>
                ...
                <Show when={state.selectedOption().availability === 'loading'}>
                  <LoaderCircleIcon
                    class='size-3.5 shrink-0 animate-spin text-muted-foreground'
                    aria-label={t.controlPanel.modelCatalogLoading()}
                  />
                </Show>
                <Show when={state.selectedOption().availability === 'unknown'}>
                  <CircleAlertIcon
                    class='size-3.5 shrink-0 text-muted-foreground'
                    aria-label={t.controlPanel.modelAvailabilityUnknown()}
                  />
