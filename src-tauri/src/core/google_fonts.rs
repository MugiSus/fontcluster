...
pub fn fetch_subset_fonts(
    ...
    let all_fonts: Vec<GoogleFontMetadata> = serde_json::from_str(&json_content).map_err(|e| {
        ...
        for &req_weight in target_weights_refs {
            ...
            if let Some(variant) = matched_variant {
                ...
                let css_res = (|| -> Result<Vec<u8>> {
                    ...
                    let css = resp.text().map_err(|e| {
                        AppError::Network(format!("Failed to read CSS text: {}", e))
                    })?;

                    // 2. Extract WOFF2 URL
                    let re = regex::Regex::new(
                        r"src:\s*url\s*\((?:'|\x22)?(https?://[^)'\x22]+)(?:'|\x22)?\)",
                    )
                    .unwrap();
                    let caps = re.captures(&css).ok_or_else(|| {
                        AppError::Processing(format!(
                            "No WOFF2 URL found in CSS for {} ({})",
                            font.family, req_weight
                        ))
                    })?;
                    ...
                    let woff2_data = woff2_resp.bytes().map_err(|e| {
                        AppError::Network(format!("Failed to read WOFF2 bytes: {}", e))
                    })?;
                    ...
                    let ttf_data = if magic == [0x77, 0x4F, 0x46, 0x32] {
                        ...
                        wuff::decompress_woff2(&woff2_data).map_err(|_| {
                            AppError::Processing(format!(
