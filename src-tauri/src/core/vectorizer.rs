...
impl Vectorizer {
    pub fn new(app: &AppHandle) -> Result<Self> {
        ...
        println!(
            "🚀 Vectorizer: loading DINOv3 model from {}",
            model_path.display()
        );
    ...
    fn process_image(&self, path: PathBuf) -> Result<()> {
        ...
        if input_offset != Some(0) {
            return Err(AppError::Processing(
                "Unexpected non-zero array offset in DINOv3 input tensor".into(),
            ));
        }
        ...
        let features = {
            ...
            let (_, output) = outputs
                .iter()
                .next()
                .ok_or_else(|| AppError::Processing("DINOv3 model returned no outputs".into()))?;
            let array = output
                .try_extract_array::<f32>()
                .map_err(|err| AppError::Processing(err.to_string()))?;
            let shape = array.shape();
            let data = array.as_slice().ok_or_else(|| {
                AppError::Processing("DINOv3 output tensor is not contiguous in memory".into())
            })?;
            match shape.len() {
                2 => {
                    let feature_dim = shape[1];
                    data[..feature_dim].to_vec()
                }
                3 => {
                ...
                rank => {
                    return Err(AppError::Processing(format!(
                        "Unsupported DINOv3 output rank: {}",
                        rank
                    )))
                }
            }
...
fn preprocess_image(path: &PathBuf) -> Result<Array4<f32>> {
    ...
    for (x, y, pixel) in resized.enumerate_pixels() {
        for channel in 0..3 {
            let normalized =
                (pixel[channel] as f32 / 255.0 - MODEL_MEAN[channel]) / MODEL_STD[channel];
...
fn resolve_model_path(app: &AppHandle) -> Result<(PathBuf, Option<TempDir>)> {
    ...
    Err(AppError::Processing(format!(
        "Could not find a DINOv3 ONNX file under src-tauri/models or bundled resources. Expected {}.onnx or model.onnx.",
        MODEL_STEM
    )))
}
...
fn find_model_file(root: &PathBuf) -> Result<Option<PathBuf>> {
    let mut direct_candidates = vec![
        root.join(format!("{MODEL_STEM}.onnx")),
        root.join("model.onnx"),
    ];

    for candidate in direct_candidates.drain(..) {
        if candidate.exists() {
        ...
        .filter_map(|entry| entry.ok())
    {
        ...
        if file_name == "model.onnx" || path_str.contains(MODEL_STEM) {
