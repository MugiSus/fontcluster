...
function getFamilyCandidates(payload) {
    return unique([
        payload.familyName,
        payload.fontName,
...
function stylePriority(style, targetWeight) {
    var _a;
    const normalizedStyle = normalizeName(style);
    const targetStyles = (_a = WEIGHT_STYLE_NAMES[targetWeight]) !== null && _a !== void 0 ? _a : [];
    const exactMatch = targetStyles.some((candidate) => normalizeName(candidate) === normalizedStyle);
    return {
...
function isApplyFontMessage(message) {
    if (!message || typeof message !== 'object')
        return false;
    return message.type === 'apply-font';
}
figma.ui.onmessage = (message) => {
    if (!isApplyFontMessage(message))
        return;
    applyFont(message.payload, message.sequence).catch((error) => {
        console.error(error);
